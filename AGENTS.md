...
Este archivo se inyecta automáticamente en cada conversación de Codex en este repo. Las reglas de abajo son **obligatorias** y no negociables.
...
La skill está en [.Codex/skills/commit/SKILL.md](.Codex/skills/commit/SKILL.md) y contiene:
...
La skill está en [.Codex/skills/pull/SKILL.md](.Codex/skills/pull/SKILL.md) y hace UNA cosa única: ejecuta `git pull origin dev` y reporta el resultado.
...
## REGLA #2.5 — Codex NUNCA debe figurar como autor ni co-autor

Los commits los firman las **personas del equipo** (Christopher y sus compañeros). Eso es normal y esperado: cualquier desarrollador humano puede ser autor. Lo **ÚNICO prohibido** es que Codex / el agente IA aparezca en la autoría.

- **NUNCA** agregues el trailer `Co-Authored-By: Codex ...` (ni ningún `Co-Authored-By` de un modelo de IA) al mensaje de commit.
...
| Si querés... | Mirá |
|---|---|
| Hacer un commit | Skill [`commit`](.Codex/skills/commit/SKILL.md) |
| Traer cambios desde dev | Skill [`pull`](.Codex/skills/pull/SKILL.md) |
